...
public class Engine implements Serializable, ITechnology {
    ...
    public static final int OS_ENHANCED_FUSION_ENGINE = 14;  // Advance Fusion (OSEnhancedFusion) – 90% weight, 1 side-torso crit, 15 free HS
    ...
    public static final int OS_ENHANCED_LIGHT_ENGINE = 27; // Advance Light (OSEnhancedLight) – 60% weight, 2 side-torso crits, 15 free HS
    ...
    private static final String[] TYPE_KEYS = {
          ...
          "OSEnhancedFusion", "OSLight", "OSEnhancedXL", "OSEnhancedXXL", "OSCompact",
          "OSSHXL", "OSSHXXL",
          "OSImproveFusion", "OSImproveLight", "OSXL", "OSImproveXL", "OSXXL",
          "OSStandardFusion", "OSEnhancedLight", "OSImproveXXL", "OSImproveCompact",
          "OSEnhancedCompact", "OSSHStandard",
          "OSImproveICE", "OSHybrid"
    };
    ...
    private boolean isValidEngine() {
        ...
        // Superheavy OS engines are allowed to exceed the normal rating table
        boolean isOSSH = (engineType == OS_SH_XL_ENGINE || engineType == OS_SH_XXL_ENGINE
              || engineType == OS_SH_STANDARD_ENGINE);
        if ((((engineRating / 5) > ENGINE_RATINGS.length) || (engineRating < 0))
              && !hasFlag(SUPPORT_VEE_ENGINE) && !isOSSH) {
        ...
        switch (engineType) {
            ...
            case OS_ENHANCED_FUSION_ENGINE:
            case OS_STANDARD_LIGHT_ENGINE:
            case OS_ENHANCED_XL_ENGINE:
    ...
    public static int getEngineTypeByString(String type) {
        ...
        // OS engine checks must precede standard checks (names contain "xl", "light", etc.).
        // The "(os)" suffix marks current OS engine names; "legion"/"adv." are kept for
        // backward compatibility with units saved before the Legion -> OS rename.
        if (lower.contains("(os)") || lower.contains("legion") || lower.contains("superhvy")
              || lower.contains("adv.")) {
            // Superheavy OS engines (most specific)
            if (lower.contains("superhvy xxl") || lower.contains("superheavy xxl")) {
                return OS_SH_XXL_ENGINE;
            ...
            if (lower.contains("hybrid")) {
                return OS_HYBRID_ENGINE;
            }
            if (lower.contains("imp. ice") || lower.contains("improve ice") || lower.contains("ice")) {
                return OS_IMPROVE_ICE_ENGINE;
            }
            ...
            if (lower.contains("compact")) {
                ...
                } else if (lower.contains("adv. compact") || lower.contains("advance compact") || lower.contains("enhanced compact")) {
                    return OS_ENHANCED_COMPACT_ENGINE;
                } else {
            ...
            if (lower.contains("adv. xxl") || lower.contains("advance xxl") || lower.contains("enhanced xxl")) {
                return OS_ENHANCED_XXL_ENGINE;
            } else if (lower.contains("adv. xl") || lower.contains("advance xl") || lower.contains("enhanced xl")) {
                return OS_ENHANCED_XL_ENGINE;
            } else if (lower.contains("adv. light") || lower.contains("advance light") || lower.contains("enhanced light")) {
                return OS_ENHANCED_LIGHT_ENGINE;
            } else if (lower.contains("adv. fusion") || lower.contains("advance fusion") || lower.contains("enhanced fusion")) {
                return OS_ENHANCED_FUSION_ENGINE;
            }
    ...
    public double getWeightEngine(Entity entity, RoundWeight roundWeight) {
        ...
        // OS Superheavy engines use a quadratic formula anchored at ENGINE_RATINGS[80]
        // (rating 400, 49.0 tons) — the practical ceiling for standard 100-ton assault mechs.
        // This gives continuous weight at rating 400 and reasonable scaling up to rating 800.
        // Formula: base = ENGINE_RATINGS[80] × (rating / 400)²
        double baseRatingWeight;
        if (engineType == OS_SH_XL_ENGINE || engineType == OS_SH_XXL_ENGINE
              || engineType == OS_SH_STANDARD_ENGINE) {
            double ratingRatio = engineRating / 400.0;
            baseRatingWeight = ENGINE_RATINGS[80] * ratingRatio * ratingRatio;
        } else {
            int idx = Math.min(100, (int) Math.ceil(engineRating / 5.0));
            baseRatingWeight = ENGINE_RATINGS[idx];
        }
        double weight = baseRatingWeight;
        switch (engineType) {
            case COMBUSTION_ENGINE:
                weight *= 2.0;
                break;
            ...
            case FUEL_CELL:
                ...
                break;
            case OS_IMPROVE_ICE_ENGINE:
                weight *= 1.7; // OS Improve ICE (non-fusion)
                break;
            case OS_HYBRID_ENGINE:
                weight *= 1.5; // OS Hybrid (non-fusion)
                break;
            case NONE:
                return 0;
            // OS engines
            ...
            case OS_STANDARD_COMPACT_ENGINE:
                ...
                break;
            case OS_SH_XL_ENGINE:
                weight *= 0.45;  // same multiplier as Advance XL
                break;
            case OS_SH_XXL_ENGINE:
            ...
            case OS_ENHANCED_COMPACT_ENGINE:
                ...
                break;
            case OS_SH_STANDARD_ENGINE:
                weight *= 0.90; // Superheavy Standard
                break;
        }
    ...
    public static Map<Integer, String> getAllEngineCodeName() {
        ...
        result.put(OS_ENHANCED_FUSION_ENGINE, getEngineTypeName(OS_ENHANCED_FUSION_ENGINE));
        result.put(OS_STANDARD_LIGHT_ENGINE, getEngineTypeName(OS_STANDARD_LIGHT_ENGINE));
        result.put(OS_ENHANCED_XL_ENGINE, getEngineTypeName(OS_ENHANCED_XL_ENGINE));
        result.put(OS_ENHANCED_XXL_ENGINE, getEngineTypeName(OS_ENHANCED_XXL_ENGINE));
        ...
        result.put(OS_ENHANCED_LIGHT_ENGINE, getEngineTypeName(OS_ENHANCED_LIGHT_ENGINE));
        ...
        result.put(OS_ENHANCED_COMPACT_ENGINE, getEngineTypeName(OS_ENHANCED_COMPACT_ENGINE));
        result.put(OS_SH_STANDARD_ENGINE, getEngineTypeName(OS_SH_STANDARD_ENGINE));
        result.put(OS_IMPROVE_ICE_ENGINE, getEngineTypeName(OS_IMPROVE_ICE_ENGINE));
        result.put(OS_HYBRID_ENGINE, getEngineTypeName(OS_HYBRID_ENGINE));
    ...
    public int[] getCenterTorsoCriticalSlots(int gyroType) {
        if (engineType == COMPACT_ENGINE || engineType == OS_STANDARD_COMPACT_ENGINE
              || engineType == OS_IMPROVE_COMPACT_ENGINE || engineType == OS_ENHANCED_COMPACT_ENGINE) {
            int[] slots;
    ...
    public int[] getSideTorsoCriticalSlots() {
        ...
        } else if (engineType == OS_ENHANCED_FUSION_ENGINE) {
        ...
        } else if (engineType == OS_ENHANCED_XL_ENGINE) {
            return hasFlag(SUPERHEAVY_ENGINE) ? new int[] { 0, 1 } : new int[] { 0, 1, 2 };  // Advance XL: 3
        } else if (engineType == OS_ENHANCED_XXL_ENGINE) {
        ...
        } else if (engineType == OS_ENHANCED_LIGHT_ENGINE) {
    ...
    /**
     * @return True if this engine cannot mount OS Triple Heat Sinks (THS). Standard-tier OS engines
    ...
    /**
     * @return True if this engine cannot mount OS Quadruple Heat Sinks (QHS). Standard- and
     *       Improve-tier OS engines lack the cooling integration required for quadruple heat sinks.
     */
    ...
    public int getBaseCost() {
        int cost = switch (engineType) {
            ...
            case FISSION -> 7500;
            // OS engine costs
            case OS_ENHANCED_FUSION_ENGINE -> 9000;    // Advance Fusion
            case OS_STANDARD_LIGHT_ENGINE -> 13000;      // Light (OS)
            case OS_ENHANCED_XL_ENGINE -> 25000;         // Advance XL
            case OS_ENHANCED_XXL_ENGINE -> 100000;       // Advance XXL
            ...
            case OS_ENHANCED_LIGHT_ENGINE -> 19000;  // OS Advance Light
    ...
    private static final TechAdvancement OS_IMPROVE_ICE_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(2850, 2860, 2875)
          .setTechRating(TechRating.D)
          .setAvailability(AvailabilityValue.C, AvailabilityValue.C, AvailabilityValue.B, AvailabilityValue.B)
          .setStaticTechLevel(SimpleTechLevel.STANDARD);

    private static final TechAdvancement OS_HYBRID_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
    ...
    // OS engine tech advancements
    // Advance Fusion (OSEnhancedFusion): Advance static level, tech rating F
    private static final TechAdvancement OS_STANDARD_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
    ...
    // Light (OSLight): Standard static level
    private static final TechAdvancement OS_LIGHT_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(2845, 2855, 2865)
          .setTechRating(TechRating.E)
          .setAvailability(AvailabilityValue.F, AvailabilityValue.C, AvailabilityValue.B, AvailabilityValue.B)
          .setStaticTechLevel(SimpleTechLevel.STANDARD);

    // Advance XL (OSEnhancedXL): Advance static level, tech rating F
    private static final TechAdvancement OS_XL_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(3000, 3025, 3050)
          .setTechRating(TechRating.F)
          .setAvailability(AvailabilityValue.F, AvailabilityValue.D, AvailabilityValue.C, AvailabilityValue.C)
          .setStaticTechLevel(SimpleTechLevel.ADVANCED);

    private static final TechAdvancement OS_XXL_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(3000, 3025, 3050)
          .setTechRating(TechRating.F)
    ...
    private static final TechAdvancement OS_IMP_STD_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(2820, 2830, 2840)
          .setTechRating(TechRating.E)
          .setAvailability(AvailabilityValue.F, AvailabilityValue.C, AvailabilityValue.B, AvailabilityValue.B)
          .setStaticTechLevel(SimpleTechLevel.STANDARD);

    // OS Improve Light (OSImproveLight): Standard static level (Improve Light is classified Standard)
    private static final TechAdvancement OS_IMP_LIGHT_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(2855, 2865, 2875)
          .setTechRating(TechRating.E)
          .setAvailability(AvailabilityValue.F, AvailabilityValue.C, AvailabilityValue.B, AvailabilityValue.B)
          .setStaticTechLevel(SimpleTechLevel.STANDARD);

    // OS XL (OSXL): Standard static level
    private static final TechAdvancement OS_BASE_XL_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(2860, 2870, 2880)
          .setTechRating(TechRating.E)
          .setAvailability(AvailabilityValue.F, AvailabilityValue.C, AvailabilityValue.B, AvailabilityValue.B)
          .setStaticTechLevel(SimpleTechLevel.STANDARD);

    // OS Improve XL (OSImproveXL): Standard static level
    private static final TechAdvancement OS_IMP_XL_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(2870, 2880, 2890)
          .setTechRating(TechRating.E)
          .setAvailability(AvailabilityValue.F, AvailabilityValue.C, AvailabilityValue.B, AvailabilityValue.B)
          .setStaticTechLevel(SimpleTechLevel.STANDARD);

    // OS XXL (OSXXL): Experimental static level (all XXL is classified Experimental)
    private static final TechAdvancement OS_BASE_XXL_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(2915, 2930, 2950)
          .setTechRating(TechRating.F)
          .setAvailability(AvailabilityValue.X, AvailabilityValue.F, AvailabilityValue.D, AvailabilityValue.C)
          .setStaticTechLevel(SimpleTechLevel.EXPERIMENTAL);

    // OS Advance Light (OSEnhancedLight): Advance static level, tech rating F
    ...
    // OS Improve XXL (OSImproveXXL): Experimental static level
    private static final TechAdvancement OS_IMP_XXL_TA = new TechAdvancement(TechBase.OUTER_SPHERE)
          .setISAdvancement(2905, 2920, 2945)
          .setTechRating(TechRating.F)
          .setAvailability(AvailabilityValue.X, AvailabilityValue.F, AvailabilityValue.D, AvailabilityValue.C)
          .setStaticTechLevel(SimpleTechLevel.EXPERIMENTAL);
    ...
    // OS Advance Compact (OSEnhancedCompact): Advance static level, tech rating F
        ...
        switch (engineType) {
            ...
            case OS_SH_STANDARD_ENGINE:
                if (year < 2800) {
                    ...
                    return TechConstants.T_OUTER_SPHERE_EXPERIMENTAL;
                }
            default:
